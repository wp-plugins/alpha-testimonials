...
!(function($){
  //testimonials
    ...
    var delay =  $('#testimonials').data('delay');
    var layout = $('#testimonials').data('layout');
    var autoplayW = $('#testimonials.tswd').data('autoplay');
    var delayW =  $('#testimonials.tswd').data('delay');
    var play = '';
    //set selected testimonial active
    var customer = $('.customer');
    var testimonials = customer.length;
    var randomActive = 0;
    
     //set random testimonal active
      $('#testimonials:not(.tswd)').find('.customer').eq(randomActive).addClass('active').find('.testimonial-bubble').addClass('fadeInDown').closest('.customer').find('.cus-profile').find('.cus-name').fadeIn(200, function(){ $(this).css('bottom', '75px')});    
      //set random testimonal active
      $('#testimonials.responsive').find('.customer').eq(randomActive).addClass('active').find('.testimonial-bubble').addClass('fadeInRight');   
      ...
      if( autoplayW === 'on' ) {
        playW = setInterval( widget , parseInt(delayW) );
      } 
      ...
      customer.mouseenter(function(){
          if( !$(this).closest('#testimonials').hasClass('responsive') ){
           ...
           $(this).find('.testimonial-bubble').removeClass('fadeOutUp').addClass('fadeInDown').closest('.customer').find('.cus-profile').find('.cus-name').fadeIn(200, function(){ $(this).css('bottom', '85px')});   
         }
      });
    ...
    function calcWidth(){
      ...
      if( $(window).outerWidth() > 1120 && layout != 'alt' ){
        responsive = false;
        $('#testimonials:not(.tswd)').css('width', '100%');
        $('#testimonials:not(.tswd)').removeClass('responsive');
        $('#testimonials:not(.tswd)').find('.testimonial-bubble').each(function(){
           $(this).css({ 'top': '-' + ( $(this).outerHeight(true) + 80 )  + 'px'});
        });
        $('#testimonials:not(.tswd)').find('.customer.active').find('.testimonial-bubble').addClass('fadeInDown');
      }
      else if( tesH > cusH || $(window).outerWidth() < 1024 ){ 
        responsive = true;
        $('#testimonials:not(.tswd)').addClass('responsive');
        $('.testimonials-line').css('height', cusH);
        setTimeout(function(){
          $('#testimonials:not(.tswd)').find('.testimonial-bubble').removeClass('fadeInDown fadeOutUp');
          $('#testimonials:not(.tswd)').find('.testimonial-bubble.active').removeClass('fadeInDown');
        },75); 
        $('#testimonials:not(.tswd).responsive').find('.testimonial-bubble').css('top', '-50' + 'px');
      } else {
        $('#testimonials:not(.tswd)').css('width', '100%');
        $('#testimonials:not(.tswd)').removeClass('responsive');
      }

      if( $('#testimonials:not(.tswd)').outerWidth(true) < 860 || layout === 'alt' ) {
          responsive = true;
          $('#testimonials:not(.tswd)').addClass('responsive');
     ...
     //integrating hammer.js
        ...
        touch.on("swipeleft", function(ev) {
            ...
            if( layout === 'default' && responsive === false ){
              ...
              testimon.find('.customer').eq(next).find('.testimonial-bubble').removeClass('fadeOutUp').addClass('fadeInDown').closest('.customer').find('.cus-profile').find('.cus-name').fadeIn(200, function(){ $(this).css('bottom', '85px')});     
            } else {
    ...
    function scroll(){ 
        var testimon = $('#testimonials:not(.tswd)');
        var last = $('#testimonials:not(.tswd)').find('.customer').length;
        var curActive = testimon.find('.customer.active').index();
        var next = curActive +1;
        if( next === last ){
          next = 0;
    ...
    function widget(){
        var testimon = $('#testimonials.tswd');
        var last = $('#testimonials.tswd').find('.customer').length;
        var curActive = testimon.find('.customer.active').index();
        var next = curActive +1;
        if( next === last ){
          next = 0;
